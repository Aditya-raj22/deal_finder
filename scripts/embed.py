"""Generate embeddings for crawled articles and store in ChromaDB.

This script:
1. Fetches articles with status='pending' from content cache
2. Generates embeddings using sentence transformers
3. Stores in ChromaDB for semantic search
4. Updates status to 'embedded' in content cache

Features:
- Resumable: Only processes articles with status='pending'
- Batch processing: Efficient batching for embedding and ChromaDB
- Progress tracking: Logs progress and checkpoints every 1000 articles
- Error handling: Tracks failed articles for retry
"""
...
def embed_articles(
    ...
    """Generate embeddings for pending articles.
    ...
    """
    logger.info("="*80)
    logger.info("EMBED STAGE: Generate Embeddings and Store in ChromaDB")
    logger.info("="*80)
    ...
    if results['failed'] > 0:
        ...
        logger.info(f"\nTo retry failed articles, run: python embed.py --retry-failed")

    service.close()


def _print_progress(progress: dict):
    """Print progress statistics in formatted way.

    Args:
        progress: Progress dict from EmbeddingService.get_progress()
    """
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Generate embeddings for crawled articles"
    )
    parser.add_argument(
        "--batch-size",
        type=int,
        default=100,
        help="ChromaDB batch size (default: 100)"
    )
    parser.add_argument(
        "--max-articles",
        type=int,
